...
for u, v, d in G.edges(data=True):
    ...
    if d.get("edge_type") == "claim-claim":
        # normalize endpoints to strings for membership check
...
for u, v, d in G.edges(data=True):
    ...
    elif et == "claim-entity" and show_all_nodes and u_str in valid_claims:
...
for cid in valid_claims:
    if not ensure_claim_node(G_sub, G, cid):
        # claim id not in full graph; skip silently
        pass
...
if focus_claim:
    ...
    if focus_key is None:
        # try to add it (if present in full graph and valid by date)
...
if G_draw.number_of_nodes() == 0:
    ...
    ax.set_title(f"Graph up to {selected_date.date()} (drawn {G_draw.number_of_nodes()} nodes / {G_draw.number_of_edges()} edges){title_focus}",
                 fontsize=14)
    ax.axis("off")
