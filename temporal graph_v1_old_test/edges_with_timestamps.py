...
def build_temporal_graph_with_edge_attrs_v1(df, entity_cols):
    df = df.sort_values("claim_date")
    G = nx.DiGraph()  # directed to reflect temporal flow

    for idx, row in df.iterrows():
        cid = row["claim_id"]
        cdate = row["claim_date"]

        G.add_node(cid, claim_date=cdate)

        history = df[(df["claim_date"] <= cdate) & (df["claim_id"] != cid)]

        for _, other in history.iterrows():
            other_cid = other["claim_id"]
            for col in entity_cols:
                if pd.notna(row[col]) and pd.notna(other[col]) and row[col] == other[col]:
                    G.add_edge(other_cid, cid,
                               via=col,
                               shared_value=row[col],
                               timestamp=cdate,
                               entity_type=col)
                    break
...
def build_temporal_graph_with_edge_attrs_undirected(df, entity_cols):
    df = df.sort_values("claim_date")
    G = nx.Graph()  # Undirected for simplicity
    seen_pairs = set()  # Avoid duplicated claim-to-claim edges
    ...
    for idx, row in df.iterrows():
        ...
        # Add entity nodes + edges (claim → entity)
        for col in entity_cols:
            val = row[col]
            if pd.notna(val):
        ...
        # Build claim–claim edges from history
        history = df[df["claim_date"] <= cdate]
        for _, other in history.iterrows():
            other_cid = other["claim_id"]
            if cid == other_cid or (other_cid, cid) in seen_pairs or (cid, other_cid) in seen_pairs:
                continue
            ...
            for col in entity_cols:
                ...
                if pd.notna(val1) and pd.notna(val2) and val1 == val2:
                    G.add_edge(cid, other_cid,
                               edge_type="claim-claim",
                               via=col,
                               shared_value=val1,
                               timestamp=cdate)
                    seen_pairs.add((cid, other_cid))
                    break  # Only one connection per pair
...
def build_temporal_graph_with_edge_attrs(df, entity_cols):
    ...
    for idx, row in df.iterrows():
        ...
        # Directed claim–claim edges based on temporal order
        history = df[df["claim_date"] < cdate]  # strictly earlier to ensure direction

        for _, other in history.iterrows():
            other_cid = other["claim_id"]
            if cid == other_cid or (other_cid, cid) in seen_pairs:
                continue
...
def compute_graph_features_from_saved_graph(graph_path: str, df, entity_cols):
    ...
    for idx, row in df.iterrows():
        ...
        # Nodos válidos hasta esta fecha
        valid_nodes = [n for n in G_full.nodes if (
                (G_full.nodes[n].get("node_type") == "claim" and n in df[df["claim_date"] <= cdate]["claim_id"].tolist()) or
                G_full.nodes[n].get("node_type") == "entity"
        )]
        G_sub = G_full.subgraph(valid_nodes).copy()
