...
def explain_solution(label: str, sum_inv: int, X: pd.DataFrame, totals: pd.Series, L_use: dict,
                     use_bucket_bounds: bool = True):
    ...
    if sum_inv == 1:
        ...
        locked = [a for a in AGENCIES if not zip_admissible(a)]
        if locked:
...
def main():
    ...
    explain_solution("Baseline", sum_inv=0, X=X0, totals=tot0, L_use=L_seed, use_bucket_bounds=True)
    ...
    explain_solution("Live (+1)", sum_inv=1, X=X1, totals=tot1, L_use=L_from_baseline,
                     use_bucket_bounds=True)
